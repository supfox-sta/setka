...
window.player = new class {
    ...
    async init(input_context) {
        ...
        try {
            this.__realAudioPlayer.setAttribute('playsinline', '')
            this.__realAudioPlayer.setAttribute('webkit-playsinline', '')
        } catch(e) {}
    ...
    initEvents() {
        this.audioPlayer.ontimeupdate = () => {
            ...
            this.__updateMobileMiniProgress(ps)
            this.__updateMobileFullProgress(time)
            this.__syncMobileUI(false)
            // Sync mobile list progress bar
            try {
                const used = document.querySelector(`.litePlayer[data-realid='${this.current_track_id}'] .lengthTrack .selectableTrack .usedPart`)
                if(used){ used.style.width = `${Math.min(100, Math.max(0, ps))}%` }
            } catch(_) {}
            this.__updateMobilePlayButtons()

            // Update MediaSession position state for iOS/Safari lockscreen timer
            try {
                if('mediaSession' in navigator && this.audioPlayer && !isNaN(this.audioPlayer.currentTime)) {
                    navigator.mediaSession.setPositionState({
                        duration: Number(current_track.length) || 0,
                        position: Number(this.audioPlayer.currentTime) || 0,
                        playbackRate: Number(this.audioPlayer.playbackRate) || 1
                    })
                }
            } catch(_) {}
        ...
        // Keep mobile mini/full play buttons in sync with real audio tag state
        ...
        const miniBtn = document.getElementById('mobile-audio-mini-btn')
        if(miniBtn) {
            miniBtn.addEventListener('click', async (e) => {
                const tgt = e.target;
                // Dismiss clicked: hide mini and do nothing else
                if(tgt && typeof tgt.closest === 'function' && tgt.closest('#ma_dismiss')) {
                    e.stopPropagation(); e.preventDefault();
                    this.__userHideMini();
                    return;
                }
                // Play icon clicked: toggle playback instead of opening full
                if(tgt && typeof tgt.closest === 'function' && tgt.closest('#ma_play')) {
                    e.stopPropagation(); e.preventDefault();
                    if(this.audioPlayer.paused){ await this.play() } else { this.pause() }
                    this.__syncMobileUI(false)
                    return;
                }
                // Otherwise open full player
                const full = document.getElementById('mobile-audio-full')
                if(full){ full.style.display = 'block' }
                const mini = document.getElementById('mobile-audio-mini')
                if(mini){ mini.style.display = 'none' }
            })
        }
        // Full controls
        const maf_close = document.getElementById('maf_close')
        const maf_prev  = document.getElementById('maf_prev')
        const maf_next  = document.getElementById('maf_next')
        const maf_play  = document.getElementById('maf_play')
        const maf_seek  = document.getElementById('maf_seek')
        if(maf_close){ maf_close.addEventListener('click', () => { 
            const full = document.getElementById('mobile-audio-full'); if(full){ full.style.display = 'none' }
            const mini = document.getElementById('mobile-audio-mini'); if(mini){ mini.style.display = 'block' }
            try{ document.body.classList.add('has-audio') }catch(e){}
        }) }
        if(maf_prev){ maf_prev.addEventListener('click', async () => { await this.playPreviousTrack() }) }
        if(maf_next){ maf_next.addEventListener('click', async () => { await this.playNextTrack() }) }
        if(maf_play){ maf_play.addEventListener('click', async () => { if(this.audioPlayer.paused){ await this.play() } else { this.pause() } }) }
        if(maf_seek){ maf_seek.addEventListener('input', (e) => { if(this.currentTrack){ const v=Number(e.target.value)/1000; this.audioPlayer.currentTime = v*this.currentTrack.length } }) }
    }
    ...
    async setTrack(id, ref = null) {
        ...
        this.__updateFace()
        u(this.audioPlayer).trigger('volumechange')
        this.__syncMobileUI(true)
        this.__revealMobileMini()
        this.__updateMediaSession()
        // Reset mobile list progress to 0 for new track
        try {
            document.querySelectorAll('.litePlayer .lengthTrack .selectableTrack .usedPart').forEach(el => el.style.width = '0%')
        } catch(_) {}
    ...
    async play() {
        ...
        // Legacy-stable behavior: pause all other audios, then play global
        try {
            document.querySelectorAll('audio').forEach(el => {
                if(el !== this.audioPlayer) el.pause()
            })
        } catch(e) {}
        try { await this.audioPlayer.play() } catch(e) { /* iOS may defer until gesture */ }
        this.__setFavicon()
        this.__updateFace()
        navigator.mediaSession.playbackState = "playing"
        this.__clearUserHideMini()
        this.__revealMobileMini()
        this.__syncMobileUI(false)
        this.__updateMediaSession()
        try {
            if('mediaSession' in navigator) {
                navigator.mediaSession.setPositionState({
                    duration: Number(this.currentTrack.length) || 0,
                    position: Number(this.audioPlayer.currentTime) || 0,
                    playbackRate: Number(this.audioPlayer.playbackRate) || 1
                })
            }
    ...
    __setMediaSessionActions() {
        navigator.mediaSession.setActionHandler('play', async () => { 
            try { await window.player.play() } catch(e) {}
            // Always sync UI/metadata even if autoplay blocked; state will flip on first gesture
            window.player.__clearUserHideMini()
            window.player.__revealMobileMini()
            window.player.__syncMobileUI(false)
            window.player.__updateMediaSession()
            // iOS: resync after short delay in case system defers playback
            setTimeout(() => {
                try {
                    window.player.__syncMobileUI(false)
                    window.player.__updateMediaSession()
        ...
        navigator.mediaSession.setActionHandler("seekto", async (details) => {
            try {
                ...
                } else if(typeof details.seekOffset === 'number') {
                    window.player.audioPlayer.currentTime = Math.max(0, Math.min(window.player.audioPlayer.duration || (window.player.currentTrack ? window.player.currentTrack.length : 0) || 0, (window.player.audioPlayer.currentTime || 0) + details.seekOffset))
                }
                window.player.__syncMobileUI(false)
                window.player.__updateMediaSession()
            } catch(_) {}
        });
        // iOS control center forward/backward buttons
        navigator.mediaSession.setActionHandler('seekforward', () => {
            try {
                const d = window.player.currentTrack ? window.player.currentTrack.length : (window.player.audioPlayer.duration || 0)
                window.player.audioPlayer.currentTime = Math.min(d || 0, (window.player.audioPlayer.currentTime || 0) + 10)
            } catch(_) {}
        })
        navigator.mediaSession.setActionHandler('seekbackward', () => {
            try {
                window.player.audioPlayer.currentTime = Math.max(0, (window.player.audioPlayer.currentTime || 0) - 10)
            } catch(_) {}
        })
    }
    ...
    __updateMediaSession() {
        ...
        const dom = document.querySelector(`.audioEmbed[data-realid='${this.current_track_id}'], .litePlayer[data-realid='${this.current_track_id}']`)
        const albumBlock = document.querySelector('.playlistBlock')
        const albumTitle = albumBlock ? albumBlock.querySelector('.playlistInfo h4') : null
        const cover = this.currentTrack.cover || (dom ? dom.getAttribute('data-cover') : null) || "/themepack/mobile_ovk/0.0.1.0/resource/icons/note.svg"
        navigator.mediaSession.metadata = new MediaMetadata({
            title: String(this.currentTrack.name || 'Untitled'),
            artist: String(this.currentTrack.performer || 'Unknown'),
            album: albumTitle ? String(albumTitle.innerText) : 'OpenVK Audios',
            artwork: [{ src: cover }]
        })
    }

    __getDomCoverFallback() {
        try {
            const dom = document.querySelector(`.audioEmbed[data-realid='${this.current_track_id}']`)
            if(dom && dom.dataset.cover) return dom.dataset.cover
        } catch(e) {}
        return "/themepack/mobile_ovk/0.0.1.0/resource/icons/note.svg"
    }

    __syncMobileUI(forceMeta = false) {
        const mini = document.getElementById('mobile-audio-mini')
        if(!mini) return
        const ct = this.currentTrack
        if(!ct) { this.__updateMobilePlayButtons(); return }
        const needMeta = forceMeta || (this.__lastSyncedId !== this.current_track_id)
        if(needMeta){
            // Derive meta from track or DOM fallback
            const dom = document.querySelector(`.audioEmbed[data-realid='${this.current_track_id}'], .litePlayer[data-realid='${this.current_track_id}']`)
            const explicitPerformer = dom?.getAttribute('data-performer') || ''
            const explicitTitle     = dom?.getAttribute('data-title') || ''
            const fallbackName = dom?.getAttribute('data-name') || ''
            const split = fallbackName.split(' — ')
            const performer = ct.performer || explicitPerformer || (split[0] || 'Unknown')
            const title     = ct.name || explicitTitle || (split.slice(1).join(' — ') || fallbackName || 'Untitled')
            const cover     = ct.cover || (dom?.getAttribute('data-cover')) || this.__getDomCoverFallback()

            const ma_cover = document.getElementById('ma_cover')
            const ma_title = document.getElementById('ma_title')
            const ma_artist= document.getElementById('ma_artist')
            if(ma_cover) ma_cover.src = cover
            if(ma_title) ma_title.textContent = title
            if(ma_artist) ma_artist.textContent = performer

            const maf_cover = document.getElementById('maf_cover')
            const maf_title = document.getElementById('maf_title')
            const maf_artist= document.getElementById('maf_artist')
            if(maf_cover) maf_cover.src = cover
            if(maf_title) maf_title.textContent = title
            if(maf_artist) maf_artist.textContent = performer

            const maf_dur = document.getElementById('maf_dur')
            if(maf_dur && this.currentTrack) maf_dur.textContent = fmtTime(this.currentTrack.length)
            const maf_seek= document.getElementById('maf_seek')
            if(maf_seek) maf_seek.value = '0'

            this.__lastSyncedId = this.current_track_id
        }
        this.__updateMobilePlayButtons()
    }

    __revealMobileMini() {
        // Do not reveal if user explicitly hid mini-player
        if(localStorage.getItem('audio.mini.hidden') === '1') return
        const mini = document.getElementById('mobile-audio-mini')
    ...
    __hideMobileMini() {
        const mini = document.getElementById('mobile-audio-mini')
        if(mini) mini.style.display = 'none'
        try { document.body.classList.remove('has-audio') } catch(e) {}
        this.__resetMobileMiniUI()
    }

    // User intent: hide mini until user clears it
    __userHideMini() {
        localStorage.setItem('audio.mini.hidden', '1')
        this.__hideMobileMini()
        try { this.pause() } catch(e) {}
    }
    __clearUserHideMini() {
        localStorage.setItem('audio.mini.hidden', '0')
    }

    __updateMobileMiniProgress(ps) {
        const el = document.getElementById('ma_progress')
        if(el) el.style.width = `${Math.min(100, Math.max(0, ps))}%`
    }

    __updateMobileFullProgress(time) {
        const cur = document.getElementById('maf_cur')
        const seek= document.getElementById('maf_seek')
        if(cur) cur.textContent = fmtTime(time)
        if(seek && this.currentTrack){ seek.value = String(Math.floor((time/this.currentTrack.length)*1000)) }
    }

    __updateMobilePlayButtons(ended = false) {
        const isPaused = ended ? true : this.audioPlayer.paused
        const ma_play = document.getElementById('ma_play')
        const maf_play= document.getElementById('maf_play')
        if(ma_play) ma_play.textContent = isPaused ? '▶' : '⏸'
        if(maf_play) maf_play.textContent = isPaused ? '▶' : '⏸'

        // Sync list play buttons state (paused class when PAUSED)
        try {
            const id = this.current_track_id
            const desktopIcon = document.querySelector(`.audioEmbed[data-realid='${id}'] .playerButton .playIcon`)
            const mobileBtn   = document.querySelector(`.litePlayer[data-realid='${id}'] .playButton`)
            if(desktopIcon){
                if(isPaused) desktopIcon.classList.add('paused'); else desktopIcon.classList.remove('paused')
            }
            if(mobileBtn){
                if(isPaused) mobileBtn.classList.add('paused'); else mobileBtn.classList.remove('paused')
            }
        } catch(_) {}
    }
    ...
    ajReveal() {
        this.is_closed = false
        if(typeof $ === 'undefined' || !$.fn || !$.fn.draggable) {
            // Skip desktop ajax player on environments without jQuery UI (e.g., mobile)
            return
        }
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if(parsed) {
        ...
        if(parsed.current_track_id) {
            // Desktop ajax mini-player only if draggable available
            if(typeof $ !== 'undefined' && $.fn && $.fn.draggable) {
            ...
            if(!window.player.audioPlayer.paused){
                window.player.__revealMobileMini()
                window.player.__updateMobileMeta()
            } else {
                window.player.__hideMobileMini()
            }
        }
    } else {
    ...
    // Bridge native audio events to mini-player state
    if(window.player && window.player.__isIOS && window.player.__isIOS()) {
        // iOS: only react to the global audio element; do not pause other tags here
        document.addEventListener('play', (e) => {
            try {
                if(e && e.target && e.target.tagName === 'AUDIO' && window.player && window.player.audioPlayer && e.target === window.player.audioPlayer) {
                    window.player.__clearUserHideMini()
                    window.player.__revealMobileMini()
                    window.player.__syncMobileUI(false)
                    window.player.__updateMediaSession()
        ...
        // Non‑iOS: also guard against rogue inline <audio> tags
        document.addEventListener('play', (e) => {
            try {
                if(e && e.target && e.target.tagName === 'AUDIO') {
                    if(window.player && window.player.audioPlayer && e.target !== window.player.audioPlayer) {
                        e.target.pause()
                        return
                    }
                    window.player.__revealMobileMini()
    ...
    // Always reset mini UI to 00:00 on load; actual time will update on first timeupdate
    try {
        if(window.player && typeof window.player.__resetMobileMiniUI === 'function') {
            window.player.__resetMobileMiniUI()
        }
    } catch(_) {}
})

// Mobile lite player: seek by tapping progress bar in list
u(document).on('click', '.litePlayer .lengthTrack .selectableTrack', (e) => {
    try {
        ...
        window.player.audioPlayer.currentTime = percent * window.player.currentTrack.length
        window.player.__syncMobileUI(false)
        window.player.__updateMediaSession()
    } catch(_) {}
})
// Touch support for seeking on iOS
u(document).on('touchstart', '.litePlayer .lengthTrack .selectableTrack', (e) => {
    try {
        if(!window.player || !window.player.currentTrack) return
...
// Helper: reset mini UI to 00:00
window.player && (window.player.__resetMobileMiniUI = function(){
    try {
        const cur = document.getElementById('maf_cur'); if(cur) cur.textContent = '00:00'
        const seek= document.getElementById('maf_seek'); if(seek) seek.value = '0'
        const bar = document.getElementById('ma_progress'); if(bar) bar.style.width = '0%'
    } catch(_) {}
})

    // Mobile lite player: tap on square play button
u(document).on('click', '.litePlayer .playButton', async (e) => {
    const node = u(e.target).closest('.litePlayer')
    const id = Number(node.attr('data-realid'))
    if(!window.player) return
    if(!id) return

    if(!window.player.hasTrackWithId(id)) {
        // Build a temporary context from visible mobile items
        window.player.tracks = []
        u('.litePlayer').nodes.forEach(el => {
            const temp = u(el)
            window.player.appendTrack({
                'id': Number(temp.attr('data-realid')),
                'available': true,
                'keys': JSON.parse(temp.attr('data-keys')),
                'length': Number(temp.attr('data-length')),
                'url': temp.attr('data-url'),
                'name': (temp.attr('data-title') || (temp.attr('data-name') || '').split(' — ').slice(-1)[0]) || temp.attr('data-name') || 'Untitled',
                'performer': (temp.attr('data-performer') || (temp.attr('data-name') || '').split(' — ')[0]) || 'Unknown',
                'cover': (temp.attr('data-cover') || '')
            })
        })
    }
    ...
    if(window.player.__revealMobileMini) window.player.__revealMobileMini()
    if(window.player.__updateMobileMeta) window.player.__updateMobileMeta()
    if(window.player.__updateMobilePlayButtons) window.player.__updateMobilePlayButtons()
    // Update big player cover from this node if present
    try { const cov = node.attr('data-cover'); if(cov) u('#bigPlayerCover').attr('src', cov) } catch(_) {}
})

// Allow clicking on cover to toggle play/pause exactly like play button
u(document).on('click', '.audioEntry .coverPh', async (e) => {
    const entry = u(e.target).closest('.audioEntry')
    const btn = entry.find('.playerButton .playIcon')
    if(btn.length > 0) {
        btn.trigger('click')
    }
})
...
u(document).on('click', '.audioEntry .playerButton > .playIcon', async (e) => {
    ...
    // Always reveal/update mobile mini after user interaction
    if(window.player.__revealMobileMini) window.player.__revealMobileMini()
    if(window.player.__updateMobileMeta) window.player.__updateMobileMeta()
    // Update big player cover from this audio entry
    try { const cov = audioPlayer.attr('data-cover'); if(cov) u('#bigPlayerCover').attr('src', cov) } catch(_) {}
    if(window.player.isAtAudiosPage()) {
...
u(document).on("click", ".musicIcon.edit-icon", (e) => {
    ...
    `, [tr("save"), tr("cancel")], [
        function() {
            ...
            const coverInput = document.querySelector('.ovk-diag-body input[name=cover]');

            const fd = new FormData();
            fd.append('name', t_name);
            fd.append('performer', t_perf);
            fd.append('genre', t_genre);
            fd.append('lyrics', t_lyrics);
            fd.append('unlisted', Number(t_unlisted));
            fd.append('explicit', Number(t_explicit));
            fd.append('hash', u("meta[name=csrf]").attr("value"));
...
u(document).on('click', `#upload_container #uploadMusic`, async (e) => {
    ...
    for(const elem of u('#lastStepContainers .upload_container_element').nodes) {
        ...
        // Attach optional cover from the last step form
        try {
