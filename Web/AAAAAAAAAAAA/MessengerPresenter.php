...
final class MessengerPresenter extends OpenVKPresenter
{
	...
	public function renderApiWriteMessage(int $sel): void
	{
		...
		$attachments = $this->postParam("attachments") ?? []; // ожидаем массив id
		...
		// Создать сообщение через существующий репозиторий Messages
		$msgId = $this->messages->createMessage(
			$this->user->identity->getId(),
			$sel,
			$content
		);

		// Привязать вложения (проверить владение)
		if (!empty($attachments)) {
			$conn = $this->messages->getConnection(); // адаптируйте к вашему репозиторному слою
			foreach ($attachments as $aid) {
				$aid = (int)$aid;
				if ($aid <= 0) continue;
				// Проверяем, что attachment существует и принадлежит текущему пользователю
				$r = $conn->query("SELECT owner_id FROM attachments WHERE id = {$aid}")->fetch();
				if ($r && (int)$r['owner_id'] === $this->user->identity->getId()) {
					$conn->query("UPDATE attachments SET message_id = {$msgId} WHERE id = {$aid}");
				}
			}
		}

		// Отправляем событие/уведомление через signaller, как было
		$this->signaler->dispatch(new NewMessageEvent($msgId, $this->user->identity->getId()));

		header("Content-Type: application/json");
		exit(json_encode(["ok" => true, "message_id" => $msgId]));
	}
}
