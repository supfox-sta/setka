...
final class SearchPresenter extends OpenVKPresenter
{
    ...
    public function renderIndex(): void
    {
        ...
        // For music search: show 1 matching artist on top, then tracks
        $this->template->topArtist = null;
        $this->template->topArtistAvatarUrl = null;
        if ($section === 'audios' && is_string($query) && trim($query) !== '') {
            try {
                $ctx = DatabaseConnection::i()->getContext();
                // Try prefix match first for better relevance
                $tbl = $ctx->table('artists')
                    ->where('name COLLATE utf8mb4_unicode_ci LIKE ?', $query . '%')
                    ->order('id DESC')
                    ->limit(1);
                $row = $tbl->fetch();
                if (!$row) {
                    // Fallback: anywhere in name
                    $row = $ctx->table('artists')
                        ->where('name COLLATE utf8mb4_unicode_ci LIKE ?', "%" . $query . "%")
                        ->order('id DESC')
                        ->limit(1)
                        ->fetch();
                ...
                if ($row) {
                    $artist = (new \openvk\Web\Models\Repositories\Artists())->get((int)$row->id);
                    $this->template->topArtist = $artist;
                    // avatar
                    try {
                        $avatarId = $artist->getAvatarPhotoId();
                        if ($avatarId) {
                            $ph = (new \openvk\Web\Models\Repositories\Photos())->get($avatarId);
                            if ($ph) $this->template->topArtistAvatarUrl = $ph->getURLBySizeId('normal');
                        }
                    } catch (\Throwable $e) {}
                }
            } catch (\Throwable $e) {}
