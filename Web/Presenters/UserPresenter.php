...
final class UserPresenter extends OpenVKPresenter
{
    ...
    public function renderSettings(): void
    {
		...
		// YooMoney
        $ymConf = OPENVK_ROOT_CONF["openvk"]["preferences"]["yoomoney"];
        $this->template->yoomoneyEnabled = !empty($ymConf["enabled"]);
        if ($this->template->yoomoneyEnabled) {
            $userId = $this->user->identity->getId();
            $this->template->yoomoney = [
                "receiver"    => $ymConf["receiver"] ?? "",
                "description" => "Пополнение аккаунта: $userId",
                "label"       => $userId,
                "successUrl"  => $ymConf["successUrl"] ?? "/settings?act=finance",
                "paymentType" => "PC"
            ];
        }
	...
	if ($this->template->mode == "finance") {
        $ymConf = OPENVK_ROOT_CONF["openvk"]["preferences"]["yoomoney"];
        $this->template->yoomoneyEnabled = (bool)($ymConf["enabled"] ?? false);
        if ($this->template->yoomoneyEnabled) {
            $userId = $this->user->identity->getId();
            $this->template->yoomoney = [
                "receiver" => $ymConf["receiver"] ?? "",
