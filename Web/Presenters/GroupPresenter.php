...
use openvk\Web\Models\Repositories\{Clubs, Users, Albums, Managers, Topics, Audios, Posts, Documents, WikiPages};
...
final class GroupPresenter extends OpenVKPresenter
{
    ...
    public function renderView(int $id): void
    {
        ...
        if (!$club) {
            ...
            if ($club->isBanned()) {
                ...
                // pinned wiki pages for materials block
                $viewer = $this->user->identity ?? null;
                $pinnedWiki = [];
                foreach ((new WikiPages())->listPinned($club->getId(), 3) as $wikiPage) {
                    if ($club->canUserAccessWiki($viewer, $wikiPage->getVisibility())) {
                        $pinnedWiki[] = $wikiPage;
                    }
                }
                $this->template->pinnedWiki = $pinnedWiki;
            }
    ...
    public function renderEdit(int $id): void
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            $wikiVisibility = (int) ($this->postParam("wiki_visibility") ?? $club->getWikiVisibility());
            if ($wikiVisibility < 0 || $wikiVisibility > 2) {
                $wikiVisibility = 0;
            }
            $club->setWikiVisibility($wikiVisibility);
