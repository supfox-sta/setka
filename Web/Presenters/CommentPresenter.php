...
final class CommentPresenter extends OpenVKPresenter
{
    ...
    public function renderMakeComment(string $repo, int $eId): void
    {
        $this->assertUserLoggedIn();
        $this->willExecuteWriteAction();
        $isAjax = (($_SERVER['HTTP_X_OPENVK_AJAX_QUERY'] ?? '0') === '1');
        ...
        if ($_FILES["_pic_attachment"]["error"] === UPLOAD_ERR_OK) {
            try {
                ...
                $this->flashFail("err", tr("error_when_publishing_comment"), tr("error_when_publishing_comment_description"), null, $isAjax);
            }
        }
        ...
        if (empty($this->postParam("text")) && sizeof($horizontal_attachments) < 1 && sizeof($vertical_attachments) < 1) {
            $this->flashFail("err", tr("error_when_publishing_comment"), tr("error_comment_empty"), null, $isAjax);
        }
        ...
        try {
            ...
            $this->flashFail("err", tr("error_when_publishing_comment"), tr("error_comment_too_big"), null, $isAjax);
        }
