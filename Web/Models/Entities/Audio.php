...
class Audio extends Media
{
    ...
    {
        $aid = $this->getRecord()->artist_id ?? null;
        return is_null($aid) ? null : (int) $aid;
    }

    public function isArtistTrack(): bool
    {
        return ((int) ($this->getRecord()->type ?? 0)) === 1;
    }

    public function setArtistId(?int $artistId): void
    {
        $this->stateChanges("artist_id", $artistId);
    }
    ...
    {
        $this->stateChanges("status", $status);
    }

    public function getStatus(): ?string
    {
        $st = $this->getRecord()->status ?? null;
        return is_null($st) ? null : (string) $st;
    }
     ...
     * Cover art helpers
     */
    public function setCoverPhoto(Photo $photo): void
    {
        $this->stateChanges("cover_photo", $photo->getId());
    }

    public function getCoverPhoto(): ?Photo
    {
        $rid = $this->getRecord()->cover_photo ?? null;
        if (!$rid) return null;
        return (new Photos())->get((int) $rid);
    }

    public function getCoverURL(): ?string
    {
        $ph = $this->getCoverPhoto();
        if ($ph) {
            return $ph->getURLBySizeId("tiny");
        }
        return null; // No cover -> let templates show note.svg fallback
    }
