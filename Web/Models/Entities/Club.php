...
class Club extends RowModel
{
    ...
    public function getWikiVisibility(): int
    {
        $record = $this->getRecord();
        return isset($record->wiki_visibility) ? (int) $record->wiki_visibility : 0;
    }

    public function setWikiVisibility(int $visibility): void
    {
        if ($visibility < 0 || $visibility > 2) {
            $visibility = 0;
        }

        $this->stateChanges("wiki_visibility", $visibility);
    }

    private function passesVisibilityLevel(int $visibility, ?User $viewer): bool
    {
        if ($visibility === 0) {
            return true;
        }
        ...
        if ($this->canBeModifiedBy($viewer)) {
            return true;
        }
        ...
        return (bool) $this->getSubscriptionStatus($viewer);
    }

    public function canUserAccessWiki(?User $viewer, ?int $pageVisibility = null): bool
    {
        if (!$this->passesVisibilityLevel($this->getWikiVisibility(), $viewer)) {
            return false;
        }

        if (!is_null($pageVisibility)) {
            return $this->passesVisibilityLevel($pageVisibility, $viewer);
        }
