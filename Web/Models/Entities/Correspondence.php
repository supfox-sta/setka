...
class Correspondence
{
    ...
    public function sendMessage(Message $message, bool $dontReverse = false)
    {
        ...
        if ($ids[0] !== $ids[1]) {
            ...
            $convId = method_exists($message, 'getConversationId') ? $message->getConversationId() : null;
            if (!is_null($convId)) {
                // broadcast to all conversation members (except sender)
                $rows = DatabaseConnection::i()->getConnection()->query('SELECT member_id FROM conversation_members WHERE conversation_id = ?', $convId);
                foreach ($rows as $r) {
                    $mid = (int)$r['member_id'];
                    if ($mid === $message->getSender()->getId()) {
                        continue;
                    }
                    (SignalManager::i())->triggerEvent($event, $mid);
                }
            } else {
