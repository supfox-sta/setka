...
class Audios
{
    ...
    public function getUserCollection(User $user, int $offset = 0, ?int $limit = null): \Traversable
    {
        return $this->getByEntityID($user->getId(), $offset, $limit);
    }
    ...
    {
        return new EntityStream("Audio", $this->audios->where("created >= " . (time() - 259200))->where(["withdrawn" => 0, "deleted" => 0, "unlisted" => 0, "status" => 'approved'])->order("created DESC")->limit(25));
    }

    public function getPopular(): EntityStream
    {
        return new EntityStream("Audio", $this->audios->where("listens > 0")->where(["withdrawn" => 0, "deleted" => 0, "unlisted" => 0, "status" => 'approved'])->order("listens DESC")->limit(25));
    }
