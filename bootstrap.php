...
function ovkGetQuirk(string $quirk): int
{
    ...
    if (!$quirks) {
        try {
            $parsed = chandler_parse_yaml(__DIR__ . "/quirks.yml");
            $quirks = is_array($parsed) ? $parsed : [];
        } catch (\Throwable $e) {
            $quirks = [];
        }
    }

    return isset($quirks[$quirk]) ? (int) $quirks[$quirk] : 0;
}
...
{
    try {
        $parsed = chandler_parse_yaml(OPENVK_ROOT . "/locales/list.yml");
    } catch (\Throwable $e) {
        $parsed = [];
    }
